...
namespace BookStore.Areas.Customer.Controllers
{
    ...
    public class HomeController : ControllerBase
    {
        ...
        {
            var books = await _bookRepository.GetAllAsync(b=>b.Discount > 0 , includes: [b=>b.Author , b=>b.Category , b=>b.Promotions]);
            if (books == null || !books.Any())
            {
                return NotFound(new ReturnModelResponse
                {
                    ReturnCode = 404,
                    ReturnMessage = "No sale books found."
                });
            }
            var saleBooksDTO = books.Select(b => new BookResponse
            {
                Id = b.Id,
                Title = b.Title,
                Description = b.Description,
                ...
                CategoryName = b.Category.Name,
                Price = b.Price,
                Promotions = b.Promotions?.Where(p=>p.ExpiryDate <= DateTime.Now.AddMinutes(30) && p.ExpiryDate > DateTime.Now)
                .Select(p => p.Code).ToList()
            }).ToList().Where(b=>b.Promotions.Any()).Take(4);
            if (saleBooksDTO is null)
            {
                return NotFound(new ReturnModelResponse
                {
                    ReturnCode = 404 ,
